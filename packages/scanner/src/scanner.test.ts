...
describe("snapshot-driven eligibility (R3)", () => {
  it("stays eligible when every authority is fresh", () => {
    const result = eligibilityFromSnapshot(freshSnapshot(), quote(0.9), { ...SNAP_GATES });
  ...
  it("fails closed on stale capability authority", () => {
    ...
    const result = eligibilityFromSnapshot(snapshot, quote(0.9), { ...SNAP_GATES });
  ...
  it("fails closed on stale proposals via the snapshot TTL", () => {
    ...
    expect(snapshot.proposalStale).toBe(true);
    const result = eligibilityFromSnapshot(snapshot, quote(null), { ...SNAP_GATES });
